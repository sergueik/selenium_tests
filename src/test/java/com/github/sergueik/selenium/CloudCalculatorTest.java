...
public class CloudCalculatorTest extends BaseTest {
	...
	public void testNestedFramePresent() {
		...
		Map<String, Object> iframesMap = new HashMap<>();
		for (WebElement iframe2 : iframes) {
			String key = String.format("id: \'%s\", src: \"%s\"",
					iframe2.getAttribute("id"), iframe2.getAttribute("src"));
			System.err.println(String.format("Found iframe (depth 2) %s", key));
			iframesMap.put(key, iframe2);
		}
	...
	public void testSwitchIntoNestedPage() {
		...
		WebDriver iframe2 = iframe.switchTo().frame(element);
		// System.err.println("Frame source:" + iframe2.getPageSource());

		List<WebElement> labels1 = iframe2
				.findElements(By.cssSelector("label[for^=\"input_\"]"));
		WebElement label1 = labels1.stream()
				.filter(o -> o.getText().contains((CharSequence) "Number of instances"))
				.collect(Collectors.toList()).get(0);
		System.err
				.println("Inut label source: " + label1.getAttribute("outerHTML"));
		WebElement input1 = iframe2.findElement(By.cssSelector(
				String.format("input[id=\"%s\"]", label1.getAttribute("for"))));
		input1.sendKeys("1");
		sleep(1000);

		List<WebElement> labels2 = iframe2
				.findElements(By.cssSelector("label[for^=\"input_\"]"));
		WebElement label2 = labels2.stream()
				.filter(o -> o.getText()
						.contains((CharSequence) "What are these instances for"))
				.collect(Collectors.toList()).get(0);
		System.err
				.println("Select label source: " + label2.getAttribute("outerHTML"));
		WebElement input2 = iframe2.findElement(By.cssSelector(
				String.format("input[id=\"%s\"]", label2.getAttribute("for"))));
		input2.sendKeys("testing");
		sleep(1000);

		List<WebElement> labels3 = iframe2
				.findElements(By.cssSelector("label[for^=\"select_\"]"));
		WebElement label3 = labels3.stream()
				.filter(o -> o.getText().contains((CharSequence) "Operating System"))
				.collect(Collectors.toList()).get(0);
		System.err
				.println("Select label source: " + label3.getAttribute("outerHTML"));
		WebElement select1 = iframe2.findElement(By.cssSelector(
				String.format("md-select[id=\"%s\"]", label3.getAttribute("for"))));
